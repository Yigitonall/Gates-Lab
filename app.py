...
# ============================================================
# PDF VE WORD RAPORLAMA KÜTÜPHANESİ
# ============================================================
...
if DOCX_ENABLED:
    def build_docx_report(report_data, antet_data):
        ...
        info_list = [
            ...
            ("Author:", antet_data.get('author', '')), 
            ("Department:", antet_data.get('department', '')), 
...
def report_info_dialog(report_data):
    ...
    st.markdown("---")
    report_format = st.radio(t("Rapor Formatı / Report Format:", "Report Format:"), ["PDF", "Word (.docx)"], horizontal=True)
    ...
    if st.button(t("✅ Raporu Oluştur", "✅ Generate Report"), use_container_width=True):
        ...
        with st.spinner(t("Rapor oluşturuluyor, grafikler işleniyor (Lütfen bekleyin)...", "Generating report, processing charts (Please wait)...")):
            try:
                if report_format == "PDF" and PDF_ENABLED:
                ...
                elif report_format == "Word (.docx)" and DOCX_ENABLED:
                    ...
                    st.error(t(f"{report_format} formatı için gerekli kütüphaneler yüklü değil.", f"Required libraries for {report_format} format are not installed."))
                    st.stop()
                ...
                st.error(t(f"Rapor Oluşturma Hatası: {e}", f"Report Generation Error: {e}"))
...
if st.session_state.app_mode == "single":
    ...
    if st.session_state.get("report_ready", False) and "report_bytes" in st.session_state:
        ...
        st.sidebar.download_button(
            ...
            file_name=f"Gates_NVH_Report_{uploaded_audio.name}.{ext}",
            mime=mime,
