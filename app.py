...
if st.session_state.app_mode == "landing":
    ...
    # Sadece Ana Menüye özel CSS
    landing_css = f"""
    {bg_css}
    <style>
    /* Scroll'u (Sayfa kaydırmayı) Kapat */
    body, html {{
    ...
    /* Butonları ve metni zorla ekranın %60'ı kadar aşağı itecek gizli blok. 
       Dil butonunu hesaba katarak biraz daralttık ki kayma olmasın. */
    .spacer-block {{
        height: 55vh;
    }}
    ...
    # Fiziksel itici blok (Her ne olursa olsun içerik bunun altında kalacak)
    st.markdown('<div class="spacer-block"></div>', unsafe_allow_html=True)
    ...
    # Dile göre başlık belirleme
    if lang == "TR":
        title_text = "GATES AR-GE GÜRÜLTÜ & AKUSTİK ANALİZ SİSTEMİ"
    else:
        title_text = "GATES R&D NVH ANALYSIS SYSTEM"

    # Başlık ve Alt Başlık (CSS ile zorla tek satır yapıldı)
    st.markdown(f"<h1 style='text-align: center; color: #252525 !important; font-size: 3rem !important; margin-bottom: 2rem; white-space: nowrap;'>{title_text}</h1>", unsafe_allow_html=True)
    
    subtitle_text = "Lütfen yapmak istediğiniz analiz tipini seçin" if lang == "TR" else "Please select the type of analysis you want to perform"
    st.markdown(f"<h3 style='text-align: center; color: #444; font-size: 1.2rem; margin-bottom: 3rem; font-weight: 400;'>{subtitle_text}</h3>", unsafe_allow_html=True)

    # Butonlar
    col1, col2, col3, col4 = st.columns([1, 4, 4, 1])
    with col2:
        btn_text_single = "🔍 TEKLİ SES ANALİZİ (Single Analysis)" if lang == "TR" else "🔍 SINGLE NOISE ANALYSIS (Standard)"
        if st.button(btn_text_single, use_container_width=True):
            st.session_state.app_mode = "single"
            st.rerun()
    with col3:
        btn_text_compare = "⚖️ A/B KARŞILAŞTIRMA ANALİZİ (Comparative Analysis)" if lang == "TR" else "⚖️ A/B COMPARATIVE ANALYSIS (Reference vs Test)"
        if st.button(btn_text_compare, use_container_width=True):
            ...
            for fig_key, title in figures_to_draw:
                if fig_key in report_data["figures"]:
                    ...
                    if diag_key in report_data["diagnostics"] and fig_key != "Color Map (A - Ref)":
                        ...
                        pdf.cell(190, 8, "KARSILASTIRMA / COMPARISON", border=1, align="C", fill=True, ln=True)
    ...
    if st.session_state.app_mode == "single":
        with st.sidebar:
            ...
            st.markdown("---")
            lang_choice = st.radio("Rapor Dili (Language):", ["TR", "EN"])
    ...
    # =====================================================================
        ...
        with st.sidebar:
            ...
            st.markdown("---")
            lang_choice = st.radio("Rapor Dili (Language):", ["TR", "EN"], key="lang_cmp")
            st.session_state.lang = lang_choice
            lang = st.session_state.lang
